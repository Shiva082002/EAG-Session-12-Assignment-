...
class ContextManager:
    ...
    def add_browser_results(self, browser_results: list):
        """Add browser operation results to session memory and context."""
        for result in browser_results:
            if isinstance(result, dict):
                self.session_memory.append({
                    ...
                    "agent": result.get("agent", "browser")
                })
                
                # If browser results contain elements or content, store in globals
                if "elements" in result:
                    self.globals["browser_elements"] = result["elements"]
                if "content" in result:
                    self.globals["browser_content"] = result["content"]
                if "url" in result:
                    self.globals["current_url"] = result["url"]
